...
contract Exchangeable  is ForceSwap{
    ...
    constructor(address _returnToken) public notZeroAddress(_returnToken) ForceSwap(_returnToken){
    ...
    function buyTokens(address _fromToken,uint256 _amount) external isConversionAllowed(_fromToken) returns(bool) {
        ICurrencyPrices currencyPrice = ICurrencyPrices(getAddressOf(CURRENCY));
        uint256 fromTokenPrice = currencyPrice.getCurrencyPrice(_fromToken);
        uint256 currentTokenPrice = currencyPrice.getCurrencyPrice(address(this));
        uint256 _assignToken = safeDiv(safeMul(_amount,fromTokenPrice),currentTokenPrice);
        ERC20(_fromToken).transferFrom(msg.sender,getAddressOf(VAULT),_amount);
        return _mint(msg.sender,_assignToken);
    }
    
    function swapTokens(address _toToken,uint256 _amount) external isConversionAllowed(_toToken) returns(bool) {
        ICurrencyPrices currencyPrice = ICurrencyPrices(getAddressOf(CURRENCY));
        ITokenVault tokenVault = ITokenVault(getAddressOf(VAULT));
        uint256 toTokenPrice = currencyPrice.getCurrencyPrice(_toToken);
        uint256 currentTokenPrice = currencyPrice.getCurrencyPrice(address(this));
        uint256 _assignToken = safeDiv(safeMul(_amount,currentTokenPrice),toTokenPrice);
    ...
    function addAllowedToken(address _which) external onlySystem() returns(bool){
