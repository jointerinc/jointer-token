...
contract AuctionFormula is SafeMath {
    ...
    function calculateLiquadityMainReserve(
        ...
        returns (uint256)
    {
        uint256 _tempContrbution =  safeDiv(safeMul(yesterDaycontibution,safeExponent(10, 6)),safeMul(_baseTokenPrice,reserveSupplyBaseToken));
        uint256 _tempSupply = safeDiv(
             yesterdayPrice,
             dayBeforyesterdayPrice
        );
        
        return safeDiv(_mainReserverAmount,safeDiv(safeMul(_tempContrbution,_tempSupply) ,safeExponent(10, 6)));
    }
}
