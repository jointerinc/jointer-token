...
contract EthPriceTicker is usingProvable,Ownable {
    ...
    constructor(address _systemAddress,address _multisigAddress,string memory _parameter) payable
        public
    ...
    function updateParameter(string calldata _parameter)external onlyAuthorized(){
    ...
    {   
        ...
        if(msg.sender != systemAddress){
            require(msg.value >= fee,"If user wants to call this they have to pay price for update");
        }
        ...
        if (fee > address(this).balance) {
            ...
            bytes32 queryId = provable_query("computation",parameters, currentGasLimit);
